...
exports.createPost = async (req, res, next) => {
    const { title, content, category, image, likes, comments } = req.body;
...
exports.showSinglePost = async (req, res, next) => {
    try {
        const post = await Post.findById(req.params.id).populate('comments.postedBy', 'name').populate('postedBy', 'name');;
        // console.log(post);
        res.status(200).json({
            success: true,
            post
